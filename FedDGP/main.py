...
if __name__ == '__main__':
    ...
    parser.add_argument('--cround', type=int, default=100, help='cycle round')
    parser.add_argument('--did', type=str, default="0")

    args = parser.parse_args()
    os.environ["CUDA_VISIBLE_DEVICES"] = args.did

    args.random_state = np.random.RandomState(1)
    set_random_seed(args.seed)

    if args.dataset in ['vlcs', 'pacs', 'officehome']:
    ...
    for a_iter in range(start_iter, args.iters):
        ...
        best_acc, best_tacc, best_changed, server_acc = evalandprint(
                args, algclass, train_loaders, val_loaders, test_loaders, SAVE_PATH, best_acc, best_tacc, server_acc,
                a_iter, best_changed)
    ...
    print(s)
    print(f"server_acc:{sum(server_acc)/len(server_acc)}")
