...
def evalandprint(args, algclass, train_loaders, val_loaders, test_loaders, SAVE_PATH, best_acc, best_tacc, server_acc, a_iter, best_changed):
    ...
    # client_acc = [0] * args.n_clients
    # for client_idx in range(args.n_clients):
    #     _, test_acc = algclass.client_eval(client_idx, test_loaders[client_idx])
    #     client_acc[client_idx]=test_acc
    # print(f"personal_acc:{sum(client_acc) / len(client_acc)}")

    for i, data in enumerate(test_loaders):
        _, acc = algclass.server_eval(data)
        server_acc[i]=acc
    print(server_acc)
    ...
    if best_changed:
        ...
        if args.alg == 'feddgp':
            for i, tmodel in enumerate(algclass.p_model):
    ...
    return best_acc, best_tacc, best_changed, server_acc
